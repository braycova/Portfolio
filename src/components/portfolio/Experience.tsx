    ...
    <Timeline color="indigo" radius="md" active={experiences.length} lineWidth={1} bulletSize={55} className="p-4 mb-4">
      {experiences.map((exp, index) => (
        <Timeline.Item key={index} bullet={ <Avatar size={55} radius="md" src={exp.logo} /> }>
          ...
          <span className="border-1 border-gray-700 dark:bg-gray-800 rounded-lg my-2 p-1 px-3 text-sm">{exp.dateRange}</span>
          ...
          <ul className="flex gap-2">
            {exp.skillsUsed.map((skill, index) => (
              <li key={index} className={`border-1 border-gray-700 rounded-lg px-2 text-sm flex justify-center items-center font-bold not-dark:text-gray-100 ${skill[1]}`}>{skill[0]}</li>
            ))}
          </ul>
        </Timeline.Item>
      ))}
